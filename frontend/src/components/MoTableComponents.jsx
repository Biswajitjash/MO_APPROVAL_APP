    ...
    <TableContainer sx={{ maxHeight: isMobile ? '75vh' : '62vh' }}>
      <Table size="small" stickyHeader sx={{ tableLayout: 'fixed' }}>
        ...
        <TableBody>
          {groups.length === 0 ? (
            <TableRow>
              <TableCell colSpan={6} align="center" sx={{ py: 4, color: '#999', fontSize: '0.75rem' }}>No records found</TableCell>
            </TableRow>
          ) : groups.map((group) => {
    ...
    <TableContainer sx={{ maxHeight: '62vh', overflowX: 'auto' }}>
      <Table size="small" stickyHeader sx={{ tableLayout: 'fixed', minWidth: 1200 }}>
        ...
        <TableBody>
          {groups.length === 0 ? (
            <TableRow>
              <TableCell colSpan={14} align="center" sx={{ py: 4, color: '#999', fontSize: '0.75rem' }}>No records found</TableCell>
            </TableRow>
          ) : groups.map((group, gi) => {
                ...
                {!isCollapsed && group.rows.map((item, idx) => {
                  ...
                  return (
                    <TableRow key={`${item.Aufnr}-${idx}`} hover sx={{ '&:nth-of-type(odd)': { backgroundColor: '#f5f8ff' }, '&:hover': { backgroundColor: '#dce8ff !important' } }}>
                      <TableCell sx={{ ...cellStyle, fontWeight: 700, color: '#37474F', fontFamily: 'monospace' }}>{item.Aufnr || '—'}</TableCell>
                      <TableCell sx={cellStyle}>
                      ...
                      <TableCell title={item.Ktext} sx={{ ...cellStyle, maxWidth: 0 }}>{item.Ktext || '—'}</TableCell>
                      <TableCell sx={cellStyle}>
                      ...
                      <TableCell sx={cellStyle}>
                        <Chip label={item.Txt04 || '—'} size="small" sx={{ fontSize: '0.52rem', height: 16, fontWeight: 600, backgroundColor: '#FFF8E1', color: '#F57F17', '& .MuiChip-label': { px: 0.5 } }} />
                      </TableCell>
                      <TableCell title={item.Txt30} sx={{ ...cellStyle, maxWidth: 0, fontWeight: 600, color: '#37474F' }}>{item.Txt30 || '—'}</TableCell>
...
export const MoMobileCard = ({ item, index }) => {
  const auartColor = AUART_MAP[item.Auart]?.color || '#607D8B';
  const auartLabel = AUART_MAP[item.Auart]?.label || item.Auart;
  return (
    <Box sx={{ border: '1px solid #e8e8e8', borderRadius: 2, p: 1.2, mb: 1, background: index % 2 === 0 ? '#f9fbff' : '#fff', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
      <Box display="flex" alignItems="center" justifyContent="space-between" mb={0.5}>
        <Typography sx={{ fontFamily: 'monospace', fontWeight: 800, fontSize: '0.78rem', color: '#1a237e' }}>{item.Aufnr}</Typography>
        <Tooltip title={`${item.Auart}: ${auartLabel}`} arrow>
          <Chip label={item.Auart} size="small" sx={{ height: 18, fontSize: '0.6rem', fontWeight: 700, backgroundColor: `${auartColor}22`, color: auartColor, border: `1px solid ${auartColor}55`, '& .MuiChip-label': { px: 0.6 } }} />
        </Tooltip>
      </Box>
      <Typography sx={{ fontSize: '0.75rem', color: '#333', fontWeight: 500, mb: 0.4 }} title={item.Ktext}>{item.Ktext || '—'}</Typography>
      <Box display="flex" alignItems="center" gap={0.6} mb={0.4}>
        <Chip label={item.Txt30 || '—'} size="small" sx={{ height: 18, fontSize: '0.58rem', fontWeight: 600, backgroundColor: '#EDE7F6', color: '#4527A0', '& .MuiChip-label': { px: 0.6 } }} />
        <Typography sx={{ fontFamily: 'monospace', fontSize: '0.58rem', backgroundColor: '#F3F4F6', color: '#374151', px: 0.5, py: 0.1, borderRadius: 0.5, fontWeight: 600 }}>{item.Stat}</Typography>
      </Box>
