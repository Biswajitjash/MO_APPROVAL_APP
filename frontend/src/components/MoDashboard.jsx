    ...
    <TableContainer sx={{ maxHeight: isMobile ? '75vh' : '62vh' }}>
      <Table size="small" stickyHeader sx={{ tableLayout: 'fixed' }}>
        ...
        <TableBody>
          {groups.length === 0 ? (
            <TableRow>
              <TableCell colSpan={7} align="center" sx={{ py: 4, color: '#999', fontSize: '0.75rem' }}>No records found</TableCell>
            </TableRow>
          ) : groups.map((group) => {
...
// ============================================================================
// STAT SUMMARY FLIP CARD  (list view ↔ donut chart, auto-flips every 10s)
// ============================================================================
...
const StatSummaryFlipCard = ({ notificationData, onCardClick }) => {
  ...
  return (
    <Box
      onClick={onCardClick}
      sx={{ height: 220, cursor: 'pointer', perspective: '1000px' }}
    >
      <Box sx={{
        ...
        {/* ── FRONT: stat list ── */}
        <Card elevation={4} sx={{ ...cardBase, background: 'linear-gradient(135deg, #18dd32 0%, #f3ae36 100%)' }}>
          <CardContent sx={{ p: 1.5, height: '100%', display: 'flex', flexDirection: 'column', boxSizing: 'border-box' }}>
            ...
            <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.45)', fontSize: '0.52rem', textAlign: 'right', mt: 0.5 }}>
              ↻ auto-flips every 10s · click to open detail
            </Typography>
          </CardContent>
        </Card>

        {/* ── BACK: donut chart ── */}
        <Card elevation={4} sx={{ ...cardBase, transform: 'rotateY(180deg)', background: 'linear-gradient(135deg, #4671e9 0%, #f85b00 100%)' }}>
          <CardContent sx={{ p: 1.5, height: '100%', display: 'flex', flexDirection: 'column', boxSizing: 'border-box' }}>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    ...
                    <RechartsTooltip
                      ...
                      contentStyle={{ backgroundColor: '#fff8', border: '1px solid #ddd', borderRadius: 6, fontSize: '0.62rem' }}
                    />
                    <Legend layout="vertical" align="right" verticalAlign="middle" iconSize={7}
                      formatter={(value) => <span style={{ fontSize: '0.56rem', color: 'white' }}>{value}</span>} />
                  </PieChart>
                </ResponsiveContainer>
              </Box>
            )}

            <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.45)', fontSize: '0.52rem', textAlign: 'right' }}>
              ↻ auto-flips every 10s · click to open detail
            </Typography>
          </CardContent>
        </Card>
    ...
    <>
      ...
      >
        ...
        <Box display="flex" flexDirection="column" gap={1.5} mb={2.5}>
          <TextField label="From Date" type="date" size="small" value={localFrom}
            onChange={(e) => setLocalFrom(e.target.value)}
            InputLabelProps={{ shrink: true }} inputProps={{ max: localTo }}
            sx={{ '& .MuiInputBase-input': { fontSize: '0.75rem' } }} />
          <TextField label="To Date" type="date" size="small" value={localTo}
            onChange={(e) => setLocalTo(e.target.value)}
            InputLabelProps={{ shrink: true }} inputProps={{ min: localFrom, max: new Date().toISOString().split('T')[0] }}
            sx={{ '& .MuiInputBase-input': { fontSize: '0.75rem' } }} />
        </Box>
...
const MoDashboard = () => {
  ...
  // ── Date range state (replaces hardcoded 90) ──
  const today0 = new Date();
  const from0  = new Date();
  from0.setDate(today0.getDate() - 90);
  const [dateFrom,     setDateFrom]     = useState(from0.toISOString().split('T')[0]);   // "YYYY-MM-DD"
  const [dateTo,       setDateTo]       = useState(today0.toISOString().split('T')[0]);  // "YYYY-MM-DD"
  ...
  const [accidentalActivity,    setAccidentalActivity]    = useState([]);
  ...
  // ── Build ISO date strings from YYYY-MM-DD inputs ──
  ...
  // ── Triggers a fresh notification fetch for the given date range ──
  ...
  // ── Called by DateRangePicker when user clicks "Apply & Refresh" ──
  ...
  const fetchMoNotificationsData = async (fromDate, toDate) => {
    try {
      ...
      if (!Array.isArray(notifications)) {
        console.warn('⚠️ Unexpected response format:', notifications);
        return;
      }
      ...
      const resolvedData   = unique.filter(item => item.stat === 'I0076' || item.stat === 'I0072');
      ...
      console.log('📋 G:', generalData.length, '| BD:', breakdownData.length, '| AMC:', amcData.length, '| AC:', accidentalData.length);
    ...
    <Box sx={{ minHeight: '100vh', background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', py: { xs: 2, sm: 3, md: 4 } }}>
      ...
      <AppBar position="static" sx={{ background: 'rgba(68, 238, 68, 0.7)', mb: 3 }}>
        <Toolbar sx={{ gap: 1 }}>
          ...
          {/* ── DATE RANGE PICKER (top-right, before logout) ── */}
          <DateRangePicker
            fromDate={dateFrom}
            toDate={dateTo}
      ...
      <Container maxWidth="xl">
        ...
        {/* ── KPI ROW: Flip Card + Approval + InProgress + Resolved ── */}
        <Fade in={!loading} timeout={800}>
          <Grid container spacing={2} mb={2} alignItems="stretch">

            {/* Stat summary flip card replaces "Notification Open" static card */}
            <Grid item xs={12} sm={6} md={3}>
              <StatSummaryFlipCard
                notificationData={notificationData}
                onCardClick={handleObservationCardClick}
              />
            </Grid>
            ...
            <Grid item xs={12} sm={6} md={3}>
              <KPICard title="Resolved" value={notifLoading ? '…' : resolved.length} unit="orders" trend="up" trendValue="0%" icon={CheckCircleIcon} color="#4CAF50" onClick={handleResolvedCardClick} />
            </Grid>
        ...
        {/* ── Activity Type + Breakdown Overview ── */}
                ...
                <Grid container spacing={1} sx={{ width: '100%' }}>
                  {[
                    ...
                    <Grid item xs={12} sm={6} key={item.key}>
                      <Paper sx={{ p: 2, background: `linear-gradient(135deg, ${item.color}15 0%, ${item.color}05 100%)`, borderLeft: `5px solid ${item.color}`, borderRight: `5px solid ${item.color}`, borderRadius: 3, transition: 'all 0.2s ease', '&:hover': { transform: 'translateY(-2px)', boxShadow: `0 4px 12px ${item.color}30` } }}>
                        <Box display="flex" alignItems="center" justifyContent="space-between">
                          <Box display="flex" alignItems="center" gap={1}>
                            <Box sx={{ width: 10, height: 10, borderRadius: '20%', backgroundColor: item.color }} />
                            <Typography variant="body2" sx={{ fontWeight: 700, color: item.color }}>{item.label}</Typography>
                          </Box>
                          <Chip label={notifLoading ? '…' : item.data.length} size="small"
                            sx={{ height: 22, minWidth: 32, fontSize: '0.7rem', fontWeight: 800, backgroundColor: item.color, color: 'white', '& .MuiChip-label': { px: 1 } }} />
                        </Box>
                  ...
                  <Grid item xs={12}>
                    <Paper sx={{ p: 1.5, background: 'linear-gradient(135deg, #37474F15 0%, #37474F05 100%)', borderLeft: '5px solid #37474F', borderRight: '5px solid #37474F', borderRadius: 3 }}>
                      <Box display="flex" alignItems="center" justifyContent="space-between">
                        <Typography variant="body2" sx={{ fontWeight: 700, color: '#37474F' }}>Total Notifications</Typography>
                        <Chip label={notifLoading ? '…' : generalActivity.length + breakDownActivity.length + amcActivity.length + accidentalActivity.length} size="small"
            ...
            <Grid item xs={12} md={6}>
              <ChartCard title="Maintenance Activity Type" height={350}>
                {notifLoading ? (
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={activityChartData.filter(d => d.value > 0)} cx="50%" cy="50%" outerRadius={100} paddingAngle={2} dataKey="value" label={({ name, value }) => `${name}: ${value}`} labelLine={false}>
                        {activityChartData.filter(d => d.value > 0).map((entry, i) => <Cell key={i} fill={entry.color} />)}
                      </Pie>
                      <RechartsTooltip formatter={(value, name) => [`${value} notifications`, name]} contentStyle={{ backgroundColor: '#f5f5f5', border: '1px solid #ddd', borderRadius: '8px', fontSize: '0.75rem' }} />
                      <Legend />
                    </PieChart>
                  </ResponsiveContainer>
                )}
      ...
      <NotificationModal open={showResolvedModal} onClose={() => setShowResolvedModal(false)}
        title="Resolved Notifications" subtitle="I0072 (Notification Completed) & I0076 (Deletion Flag)"
        data={resolved} headerColor="linear-gradient(135deg, #4CAF50 0%, #2E7D32 100%)"
        icon={CheckCircleIcon} isMobile={isMobile} />
...
//       ...item,
//       qmdat: item.qmdat
//         ? new Date(parseInt(item.qmdat.replace('/Date(', '').replace(')/', '')))
