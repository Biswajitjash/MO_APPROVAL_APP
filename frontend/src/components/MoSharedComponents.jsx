...
// ── KPI Card ──────────────────────────────────────────────────────────────────
// FIX: `isPositive` logic was inverted — `trend !== 'up'` means "down" is positive,
// which shows green for downward trend. Corrected to `trend === 'up'`.
  ...
  return (
    <Card
      ...
      sx={{
        ...
        '&::before': {
          content: '""', position: 'absolute', top: -30, right: -50,
          width: 150, height: 140, background: '#1bca38',
          borderRadius: '70%', opacity: 0.2,
        },
      }}
    >
      <CardActionArea sx={{ height: '100%' }}>
        <CardContent sx={{ p: 3 }}>
          ...
          {/* Only render trend badge when trendValue is actually provided */}
          {trendValue && (
            <Box
              display="flex"
              alignItems="center"
              ...
              sx={{
                ...
                px: 0.5, py: 0.5, borderRadius: 1, width: 'fit-content',
              }}
            >
...
export const ChartCard = ({ title, children, height = 300 }) => (
  ...
  >
    <Typography variant="h6" sx={{ fontWeight: 700, mb: 2, color: '#333', flexShrink: 0 }}>
      {title}
    </Typography>
    <Box
      flex={1}
...
export const DateRangePicker = ({ fromDate, toDate, onApply, notifLoading }) => {
  ...
  // FIX: Syncs local state to latest parent values when the popover opens,
  // so re-opening always shows the currently applied range — not a stale one.
  ...
  const handleApply = useCallback(() => {
    ...
    handleClose();
  }, [onApply, localFrom, localTo, handleClose]);

  // FIX: today's date computed once per preset click — not captured in closure
  ...
  // FIX: todayMax computed once, not inline in JSX, avoids re-creation every render
  ...
  return (
    <>
      <Tooltip title="Change date range">
        ...
        >
          {/* FIX: Button had no visible label text — added the date range as label */}
          {fromDate} → {toDate}
        </Button>
      </Tooltip>
      ...
      <Popover
        ...
        PaperProps={{
          sx: { borderRadius: 3, p: 2.5, minWidth: 300, boxShadow: '0 8px 32px rgba(0,0,0,0.18)' },
        }}
      >
        ...
        <Box display="flex" flexDirection="column" gap={1.5} mb={2.5}>
          ...
          <TextField
            ...
            InputLabelProps={{ shrink: true }}
            inputProps={{ min: localFrom, max: todayMax }}
            sx={{ '& .MuiInputBase-input': { fontSize: '0.75rem' } }}
          />
        </Box>
        ...
        <Box display="flex" gap={1} justifyContent="flex-end">
          <Button
            size="small"
            onClick={handleClose}
            sx={{ fontSize: '0.7rem', textTransform: 'none', color: '#666' }}
          >
            Cancel
          </Button>
          <Button
            ...
            sx={{
              fontSize: '0.7rem', textTransform: 'none', borderRadius: 2,
